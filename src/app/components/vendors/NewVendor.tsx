...
export function NewVendor() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      navigate("/people/vendors");
    } catch (err) {
      if (err instanceof ApiError && err.validationErrors) {
        setErrors((current) => ({
          ...current,
          supplierCode: err.validationErrors?.supplierCode || current.supplierCode,
        ...
        <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
              This form now matches the ERP supplier contract around supplier code, legal identity,
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          <div className="grid gap-4 md:grid-cols-2">
            <label>
              ...
              />
              {errors.supplierCode && <div className="mt-2 text-sm text-rose-600">{errors.supplierCode}</div>}
            </label>
            <label>
              <div className="mb-2 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">
                Supplier Name
      ...
      <aside className="space-y-5">
        <section className="rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600">
            <li>Supplier code is mandatory in phase 1.</li>
