...
export function SalesReturns() {
  ...
  const returnColumns = useMemo<DataTableColumn<SalesReturnSummaryResponse>[]>(
    () => [
      {
        key: "return",
        header: "Return",
        value: (item) => `${item.returnNumber} ${item.originalSalesInvoiceId}`,
        render: (item) => (
          <div>
            <div className="text-base font-semibold text-slate-950">{item.returnNumber}</div>
            <div className="mt-1 text-sm text-slate-500">Original invoice #{item.originalSalesInvoiceId}</div>
          </div>
        ),
      },
      {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-6 xl:grid-cols-[520px_minmax(0,1fr)] 2xl:grid-cols-[560px_minmax(0,1fr)]">
        <SurfaceCard as="form" onSubmit={handleCreateReturn}>
